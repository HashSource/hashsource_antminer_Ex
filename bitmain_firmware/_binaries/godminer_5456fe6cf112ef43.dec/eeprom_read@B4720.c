int __fastcall eeprom_read(unsigned int a1, __int16 a2, int a3, int a4)
{
  ...
  {
    v12 = -2147483391;
    v18 = snprintf(s, 0x800u, "%s: Bad eeprom param, input chain is %d\n", "eeprom_read", a1);
    V_LOCK(v18);
  ...
  v5 = 2 * a1;
  LOWORD(v7) = a2;
  if ( !dword_16C564[2 * a1 + 19] )
  {
    v12 = eeprom_open(a1);
    if ( v12 < 0 )
    {
      v16 = snprintf(s, 0x800u, "%s: auto exec eeprom_open, but chain %d open eeprom failed\n", "eeprom_read", a1);
      V_LOCK(v16);
  ...
  fd_from_ctx = i2c_get_fd_from_ctx(dword_16C564[v5 + 18]);
  if ( a4 )
  {
    ...
    v12 = -2147483392;
    v13 = snprintf(s, 0x800u, "fail to read eeprom by iic, chain: %d, addr: %d\n", a1, v7);
    V_LOCK(v13);
