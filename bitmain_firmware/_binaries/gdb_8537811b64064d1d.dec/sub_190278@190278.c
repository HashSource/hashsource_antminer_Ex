void __fastcall sub_190278(int a1)
{
  void *v1; // r0
  _DWORD *v2; // r0
  void *v3; // [sp+10h] [bp-64h]
  _DWORD *v4; // [sp+14h] [bp-60h]
  _DWORD v5[5]; // [sp+18h] [bp-5Ch] BYREF
  int v6; // [sp+2Ch] [bp-48h]
  int v7; // [sp+30h] [bp-44h]
  int v8; // [sp+34h] [bp-40h]
  int s[15]; // [sp+38h] [bp-3Ch] BYREF

  if ( dword_487930 )
  {
    v2 = (_DWORD *)sub_242FF0(a1);
    sub_2594D8(*v2, "infrun: wait_for_inferior ()\n");
  }
  v3 = sub_92564((int)sub_187148, 0);
  v4 = sub_92564((int)dword_23F76C, (int)&dword_46BBCC);
  do
  {
    v6 = dword_46BBCC;
    v7 = dword_46BBD0;
    v8 = dword_46BBD4;
    v1 = memset(s, 0, 0x38u);
    dword_4896D8 = 1;
    sub_2348AC(v1);
    sub_185CE8((int)v5, v6, v7, v8, &s[4], 0);
    s[0] = v5[0];
    s[1] = v5[1];
    s[2] = v5[2];
    if ( dword_487930 )
      sub_187F28(v6, v7, v8, s[0], s[1], s[2], (int)&s[4]);
    sub_18EF6C(s);
  }
  while ( s[12] );
  sub_92668(v4);
  sub_92648(v3);
}
