...
LABEL_3:
    if ( off_46D5A4[0][3] != (char *)v1 )
      sub_946D8("the current range check setting does not match the language.\n");
    return;
  }
  if ( strcmp(v0, "auto") )
  {
    sub_94728((int)"language.c", 282, "Unrecognized range check setting: \"%s\"", v0);
    v11 = v2;
    ...
    while ( 1 )
    {
      ...
      {
        sub_94728((int)"language.c", 210, "Couldn't find language `%s' in known languages list.", v3);
        goto LABEL_22;
      }
    }
    if ( *(_DWORD *)(v5 + 8) != 1 )
