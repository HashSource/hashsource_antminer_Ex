int *__fastcall sub_1A7C80(int *result, int a2)
{
  int *v3; // r4
  int v4; // r3
  const char *v5; // r6
...
LABEL_3:
    if ( a2 )
      ...
      v6 = dword_487AC4;
    result = (int *)sub_1A75F8(v5, *(_DWORD **)(v6 + 12), 1, 10);
    v7 = result;
    if ( result )
    {
      v8 = (int *)result[2];
      result = (int *)sub_1A75F8("__GDB_EFI_INFO__", v8, 4, 1);
      if ( result )
      {
        v9 = (int **)v3;
        v10 = (_DWORD *)result[2];
      ...
      {
        result = (int *)sub_1780EC(dword_487ABC);
        v7[6] = result[17];
      }
    }
    else if ( dword_47AC88 > 0 )
    {
      return sub_F43DC(&off_46D334, "PDR for %s, but no symbol", v5);
    }
    return result;
  }
  if ( dword_47AC88 > 0 )
    return sub_F43DC(&off_46D334, "can't handle PDR for static proc at 0x%lx", *result);
  return result;
}
