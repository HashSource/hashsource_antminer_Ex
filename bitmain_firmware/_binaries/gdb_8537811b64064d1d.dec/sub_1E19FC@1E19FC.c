int __fastcall sub_1E19FC(const char *a1, int a2)
{
  int v3; // r5
  int v4; // r7
  int result; // r0
  ...
  v4 = *(_DWORD *)(a2 + 8);
  if ( !a1 )
    sub_94708("Argument required (integer, `fixed' or `limited').");
...
LABEL_5:
    if ( !v4 )
    {
      result = sub_25A40C(
                 ...
                 *(const char **)a2,
                 v3);
      if ( !result )
        sub_94708("Packet size not changed.");
    }
    goto LABEL_6;
  }
  ...
  if ( result )
  {
    ...
    if ( result )
    {
      ...
      v3 = result;
      if ( endptr == a1 )
        sub_94708("Invalid %s (bad syntax).", *(const char **)a2);
      if ( result <= 0 )
        v3 = 0x4000;
      v7 = v4;
      if ( !v4 )
