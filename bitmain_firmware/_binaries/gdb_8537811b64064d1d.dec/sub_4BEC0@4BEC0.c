int sub_4BEC0()
{
  ...
  if ( v0 == -1 )
    sub_258B2C("fork");
  v2 = sub_4C3A8(v0, &v5, 0);
  if ( v2 == -1 )
    sub_258B2C("waitpid");
  if ( v2 != v1 )
    sub_94708("linux_check_ptrace_features: waitpid: unexpected result %d.", v2);
  if ( (unsigned __int8)v5 != 127 )
    sub_94708("linux_check_ptrace_features: waitpid: unexpected status %d.", v5);
  ...
  if ( !ptrace(PTRACE_SETOPTIONS, v1, 0, 2) )
  {
    ...
    if ( ptrace(PTRACE_CONT, v1, 0) )
      sub_946D8("linux_test_for_tracefork: failed to resume child");
    v4 = sub_4C3A8(v1, &stat_loc, 0);
    if ( v1 == v4 && (unsigned __int8)stat_loc == 127 && stat_loc >> 16 == 1 )
    {
      ...
      if ( !ptrace(PTRACE_GETEVENTMSG, v1, 0, &pid) && pid )
      {
        ...
        if ( ptrace(PTRACE_KILL, pid, 0, 0) )
          sub_946D8("linux_test_for_tracefork: failed to kill second child");
        sub_4C3A8(pid, &stat_loc, 0);
      }
    }
    else
    {
      sub_946D8("linux_test_for_tracefork: unexpected result from waitpid (%d, status 0x%x)", v4, stat_loc);
    }
  }
  ...
  do
  {
    if ( ptrace(PTRACE_KILL, v1, 0) )
      sub_946D8("linux_check_ptrace_features: failed to kill child");
    result = sub_4C3A8(v1, &v5, 0);
  }
