int __fastcall sub_248C0C(int a1)
{
  const char **v1; // r6
  const char **v2; // r9
  const char *v3; // r1
  ...
  if ( a1 )
  {
    ...
    while ( v2 != v1 )
    {
      while ( 1 )
      {
        ...
        if ( *v4 == 36 )
          break;
        sub_946D8("Name \"%s\" not prefixed with '$', ignoring", v3);
LABEL_5:
        if ( v2 == v1 )
          goto LABEL_16;
      }
      ...
      {
LABEL_17:
        sub_946D8("No trace variable named \"$%s\", not deleting", v5);
        goto LABEL_5;
      }
      v8 = dword_48A910;
      v9 = 0;
...
LABEL_16:
    sub_243C5C();
    return sub_31DAF8(v26);
  }
  else
  {
    if ( sub_25A40C("Delete all trace state variables? ") )
    {
      if ( dword_48A910 )
