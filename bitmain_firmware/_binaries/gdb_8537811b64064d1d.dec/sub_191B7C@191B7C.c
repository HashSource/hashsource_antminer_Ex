const char *__fastcall sub_191B7C(const char *s2, int a2)
{
  __int64 v2; // r4
  const char *v3; // r6
  int v4; // r3
  ...
  if ( dword_4879C4 != dword_4879C8 )
  {
    ...
    do
    {
      ...
      if ( !s2 )
      {
        sub_94728((int)"interps.c", 117, "interpreter factory already registered: \"%s\"\n", v3);
        JUMPOUT(0x191C2C);
      }
      LODWORD(v2) = v2 + 8;
    }
  ...
  if ( HIDWORD(v2) == dword_4879CC )
    return (const char *)sub_191C50(&dword_4879C4, HIDWORD(v2), &v6, &v5);
  if ( HIDWORD(v2) )
  {
    v4 = v6;
