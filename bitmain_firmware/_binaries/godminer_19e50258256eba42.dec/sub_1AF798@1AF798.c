int __fastcall sub_1AF798(int a1)
{
  ...
  for ( i = 0; i <= 6; ++i )
  {
    if ( i != 3 )
    {
      ...
      {
        V_LOCK();
        logfmt_raw(v5, 0x1000u, 0, "%s failed, set reg:%02x", "ChipSetting_pmdata_RVN", HIWORD(s[2]));
        V_UNLOCK();
        zlog(
          g_zc,
