void __fastcall __noreturn open_cores_by_work_num(int a1)
{
  _DWORD *v1; // lr
  int v2; // r1
  int v3; // r2
  ...
  int v6; // r2
  int v7; // r3
  _BYTE v9[48]; // [sp+1Ch] [bp-1430h] BYREF
  _DWORD v10[8]; // [sp+101Ch] [bp-430h] BYREF
  _BYTE v11[1024]; // [sp+103Ch] [bp-410h] BYREF
  void *s; // [sp+143Ch] [bp-10h]
  unsigned int j; // [sp+1440h] [bp-Ch]
  unsigned int i; // [sp+1444h] [bp-8h]

  V_LOCK();
  logfmt_raw(v9, 0x1000u, 0, "%s...", "open_cores_by_work_num");
  V_UNLOCK();
  zlog(
    g_zc,
  ...
  while ( 1 )
  {
    for ( i = 0; i <= 0xF; ++i )
    {
      str2hex((int)v10, (int)&v11[64 * i], 64);
      memset(s, 0, 0x59u);
      v1 = (char *)s + 16;
      v2 = v10[1];
      v3 = v10[2];
