int __fastcall sub_157400(int a1)
{
  ...
  for ( i = 0; i <= 6; ++i )
  {
    if ( i != 3 )
    {
      ...
      {
        V_LOCK();
        logfmt_raw(v5, 0x1000u, 0, "%s failed, set reg:%02x", "ChipSetting_pmdata_HNS_2130", HIWORD(s[2]));
        V_UNLOCK();
        zlog(
          g_zc,
      ...
      {
        v6 = 0;
        sub_151F60(&v6, *((_DWORD *)ptr + 3 * j));
        if ( *((_WORD *)ptr + 6 * j + 3) == 5 )
        {
          ++v20;
          V_LOCK();
