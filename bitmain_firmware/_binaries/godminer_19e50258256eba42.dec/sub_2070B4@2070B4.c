int __fastcall sub_2070B4(_BYTE *a1, char *a2)
{
  ...
  if ( v9 && (n & 0x80000000) == 0 )
  {
    ...
    else
    {
      snprintf(
        v6,
        0x800u,
        "EEPROM encode eeprom_key_version or eeprom_encryption_algorithm error! eeprom_key_version:%d, eeprom_encryption_algorithm:%d",
        v14,
        v13);
      V_LOCK();
  ...
  else
  {
    snprintf(
      v6,
      0x800u,
      "EEPROM encode info error! fixture_len:%d, fixture_without_header_len:%d, fixture_actual_len:%d",
      v9,
      n,
      v12);
    V_LOCK();
