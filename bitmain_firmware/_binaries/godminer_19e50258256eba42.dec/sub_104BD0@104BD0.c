int __fastcall sub_104BD0(int a1, char a2, __int16 a3, __int16 a4, char a5, _BYTE *a6)
{
  int v6; // r3
  unsigned int v7; // r0
  int v8; // r5
  ...
  logfmt_raw(
    ...
    v26,
    "%s core_num %d",
    "ChipSetting_check_single_chip_2282_dag_reg_ETH",
    v40);
  V_UNLOCK();
  zlog(
    g_zc,
  ...
  if ( !v38 && v40 == v23 )
  {
    for ( i = 0; i < v40; ++i )
    {
      if ( a5 )
      {
        if ( a5 == 1 )
        {
          if ( (sub_FC76C(*((_DWORD *)ptr + 3 * i)) & 1) == 1 )
          {
            V_LOCK();
